		...
		if(t.isVariableDeclaration(path.node) && path.node.declarations.length == 1){
			...
			function loopFunction(startVal){
				...
				while(eval(whileCond)){
					...
					if(stringFunc.length < 2){
						let block = cases.consequent.find(x => t.isBlockStatement(x))
						let tempCode = original.slice(block.start, block.end)
						let matches = tempCode.match(new RegExp(`${objName}\\[([A-z]|[0-9])+\\[([A-z]|[0-9])+\\]\\]=function\\(\\){.*?}`, "g"))
						if(block.body.length == 2 && matches != null){
							try {
								let ast3 = parser.parse(tempCode)
								traverse(ast3, {
									"ReturnStatement": function(path){
										if(path.node.argument?.params?.length == 0) return;
										let funcBody = path.find(t.isFunctionExpression)
										funcBody.node.body.body = funcBody.node.body.body.filter(x => !t.isExpressionStatement(x))
										funcBody.node.id = t.Identifier(stringFunc.length == 0 ? "first" : "second")
										ast3.program.body = [funcBody.node]
									}
								})
								stringFunc.push(eval(`global.${(stringFunc.length == 0 ? "first" : "second")}=` + generator(ast3).code))
							}catch(e){
								// console.log(e)
								// console.log(original.slice(cases.start, cases.end))
							}
					...
					var callExp = body.filter(x => t.isCallExpression(x))
...
var _first = funcTypes.find(x => x.includes(callingFunc))
traverse(_testCode, {
	CallExpression: function(path){
		...
		try {
			if(argLen == 4){
				eval("xx." + callee.property.name + "=" + stringFunc[1].toString())
				var val = eval(str)
				...
				eval("xx." + callee.property.name + "=" + stringFunc[0].toString())
				//Since the switch case changes the string function, we need to run the original string function again
				eval("global." + _first)
				var val = eval(str)
				// console.log(val)
				if(typeof val == "string"){
...
//Change xxx["prop"]["prop"] -> xxx.prop.prop
traverse(_testCode, {
	"MemberExpression": function(path){
		try {
			// Special thanks to https://github.com/SteakEnthusiast/Supplementary-AST-Based-Deobfuscation-Materials/blob/master/Bracket%20Notation%20to%20Dot%20Notation/deobfuscator.js#L18
